...
public class ScaleSlider extends AbstractSliderButton {
    ...
    public ScaleSlider(int x, int y, int width, String label, float initialValue, Consumer<Float> onChanged) {
        this(x, y, width, label, initialValue, 0.0f, 3.0f, onChanged);
    }

    // Full constructor with custom range
    public ScaleSlider(int x, int y, int width, String label, float initialValue, float min, float max, Consumer<Float> onChanged) {
